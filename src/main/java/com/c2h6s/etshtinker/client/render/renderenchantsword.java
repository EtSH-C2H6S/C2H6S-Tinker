...
import static com.c2h6s.etshtinker.init.ItemReg.etshtinkerItems.enchantedsword;

public class renderenchantsword extends EntityRenderer<ItemProjectile> {
    private final ItemRenderer itemRenderer;
    public renderenchantsword(EntityRendererProvider.Context p_174008_) {
        super(p_174008_);
    ...
    public void render(ItemProjectile entity, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        if (entity.tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) < 12.25D)) {
            ...
            matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(90+(entity.tickCount + partialTicks) * 10 % 360));
            matrixStackIn.translate(-0.03125, -0.09375,0);
            this.itemRenderer.renderStatic(new ItemStack(enchantedsword.get()), ItemTransforms.TransformType.GROUND, packedLightIn, OverlayTexture.NO_OVERLAY, matrixStackIn, bufferIn, entity.getId());
    ...
    @Override
    public ResourceLocation getTextureLocation(ItemProjectile renderenchantsword) {
