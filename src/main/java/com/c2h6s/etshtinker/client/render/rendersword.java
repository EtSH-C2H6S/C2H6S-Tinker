...
public class rendersword extends EntityRenderer<ItemProjectile> {
    private final ItemRenderer itemRenderer;
    public rendersword(EntityRendererProvider.Context p_174008_) {
        super(p_174008_);
    ...
    public void render(ItemProjectile entity, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        if (entity.tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) < 12.25D)) {
            ...
            matrixStackIn.mulPose(Vector3f.ZP.rotationDegrees(Mth.lerp(entityYaw, entity.xRotO, entity.getXRot())-135+entity.Xdeg));
