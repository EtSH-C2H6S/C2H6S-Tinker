        ...
        if (tool.getModifierLevel(this)>0){
            ...
            if (entity instanceof LivingEntity attacker&& !(entity instanceof Player)) {
                if (target instanceof Player player && modifier.getLevel()>0) {
                    ...
                    attacker.forceAddEffect(new MobEffectInstance(etshtinkerEffects.ionized.get(),1400,2*lvl000,false,false),attacker);
                    attacker.forceAddEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 1400, 2*lvl000, false, false), attacker);
                }
            }
        }
    }
