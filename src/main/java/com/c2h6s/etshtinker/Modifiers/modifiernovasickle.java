...
public class modifiernovasickle extends etshmodifieriii implements RequirementsModifierHook ,GeneralInteractionModifierHook{
    ...
    public Component requirementsError(ModifierEntry entry) {
        return Component.translatable("recipe.etshtinker.modifier.modifiernovasickle");
    }
    ...
    public void modifierOnInventoryTick(IToolStackView tool, ModifierEntry modifier, Level level, LivingEntity livingEntity, int itemSlot, boolean isSelected, boolean isCorrectSlot, ItemStack itemStack) {
        ...
        if (toolData.getInt(stealth)<640){
            toolData.putInt(stealth,toolData.getInt(stealth)+10);
        }
    }
    ...
    public void onFinishUsing(IToolStackView tool, ModifierEntry modifier, LivingEntity entity) {
        ...
        if (!tool.isBroken() && entity instanceof Player player&&tool.getModifierLevel(this.getId())>0) {
            novasickleEntity entity1 = new novasickleEntity(etshtinkerEntity.novascikle.get(), player.getX(),player.getEyeY(),player.getZ(),player.level);
            ...
            if (tool.getModifierLevel(etshtinkerModifiers.creativenovasickle_STATIC_MODIFIER.get())>0){
                ...
                entity1.getpowah((int)Math.max(10,toolData.getInt(stealth)*0.1125));
                entity1.setDamage((float) (tool.getStats().getInt(ToolStats.ATTACK_DAMAGE)*Math.max(1,Math.pow(0.004*toolData.getInt(stealth),1.75))));
            }
